document.getElementById('run-script-base64-btn').addEventListener('click', function () {
    ...
    if (!token) {
        document.getElementById('script-output-base64').innerHTML = `<strong>Erreur :</strong> <span class="text-danger">Veuillez entrer un jeton valide !</span>`;
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                ...
                if (data.data && data.data.code_obtenu) {
                    ...
                    document.getElementById('script-output-base64').innerHTML += `
                        <br>
                        <strong>Code obtenu :</strong>
                        <strong class="bg-success text-white">${code}</strong>
                        <button type="button" class="btn btn-primary" id="copy-script-base64-btn">
                            Copier
                        </button>`;
                    ...
                    document.getElementById('copy-script-base64-btn').addEventListener('click', function () {
                        ...
                        navigator.clipboard.writeText(code).then(() => {
                            ...
                            const copyBtn = document.getElementById('copy-script-base64-btn');
