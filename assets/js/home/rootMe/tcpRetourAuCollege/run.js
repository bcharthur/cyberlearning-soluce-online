document.getElementById('run-script-tcpRetourAuCollege-btn').addEventListener('click', function () {
    ...
    const token = document.getElementById('token-tcpRetourAuCollege-input').value;  // Récupérer le jeton

    // Si le champ du jeton est vide, afficher un message dans la sortie au lieu d'une alerte
    if (!token) {
        document.getElementById('script-output-tcpRetourAuCollege').innerHTML = `<strong>Erreur :</strong> <span class="text-danger">Veuillez entrer un jeton valide !</span>`;
            ...
            if (data.status === 'success') {
                ...
                if (data.data && data.data.code_obtenu) {
                    ...
                    document.getElementById('script-output-tcpRetourAuCollege').innerHTML += `
                        ...
                        <button type="button" class="btn btn-primary" id="copy-script-tcpRetourAuCollege-btn">
                            Copier
                        </button>`;

                    // Ajouter l'événement pour le bouton de copie
                    document.getElementById('copy-script-tcpRetourAuCollege-btn').addEventListener('click', function () {
                        // Copier le code dans le presse-papier
                        navigator.clipboard.writeText(code).then(() => {
                            // Changer le style et le texte du bouton en "Copié !"
                            const copyBtn = document.getElementById('copy-script-tcpRetourAuCollege-btn');
                    ...
                    document.getElementById('script-output-tcpRetourAuCollege').innerHTML = `<strong>Infos :</strong> Erreur dans le Jeton`;
                }
            } else {
