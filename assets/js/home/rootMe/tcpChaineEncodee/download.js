document.getElementById('btn-download-tcpChaineEncodee').addEventListener('click', function () {
    ...
    fetch(`/get-script-tcpChaineEncodee?script_name=${encodeURIComponent(scriptName)}`)
        .then(response => {
            if (!response.ok) {
